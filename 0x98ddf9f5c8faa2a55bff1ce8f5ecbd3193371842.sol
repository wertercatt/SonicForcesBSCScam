...
// SONIC FORCES with Governance.
contract SONICFORCES is BEP20 {
    ...
    // Min amount to liquify. (default 500 SONICs)
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "SONIC::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the SONIC Token contract.
     */
    constructor() public BEP20("SONIC FORCES", "SONIC") {
    ...
    /// @dev overrides transfer function to meet tokenomics of SONIC
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "SONIC::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "SONIC::transfer: Tax value invalid");
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "SONIC::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "SONIC::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "SONIC::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
    ...
    function updateSONICRouter(address _router) public onlyOperator {
        ...
        require(SONICPair != address(0), "SONIC::updateSONICRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "SONIC::transferOperator: new operator is the zero address");
    ...
    */
    ...
    uint256 private salePrice = 1000000;// 0.01*10000000=10.000 token for 0.01 bnb (set as per requirements)
